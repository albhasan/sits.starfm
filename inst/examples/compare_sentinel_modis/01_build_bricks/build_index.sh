...
if [ "${index}" == "evi_10m" ]; then
    ...
    calc="'(10000 * 2.5 * (A - B) / (A + 6.0 * B - 7.5 * C + 10000)).astype(int16)'"
    parallel  --colsep ' ' /usr/bin/gdal_calc.py --quiet -A {3} -B {2} -C {1}  --outfile={5} --calc="${calc}" --NoDataValue=-9999 --type='Int16' --creation-option='COMPRESS=LZW' --creation-option='BIGTIFF=YES' --overwrite  ::: "${parameters[@]}" 

elif [ "${index}" == "ndmi_10m" ]; then

    #NOTE: A = B08_10m, B = B11_10m
    calc="'((A - B)/(1.0 * A + B) * 10000).astype(int16)'"
    parallel  --colsep ' ' /usr/bin/gdal_calc.py --quiet -A {3} -B {4} --outfile={5} --calc="${calc}" --NoDataValue=-9999 --type='Int16' --creation-option='COMPRESS=LZW' --creation-option='BIGTIFF=YES' --overwrite  ::: "${parameters[@]}" 

elif [ "${index}" == "ndvi_10m" ]; then

    #NOTE: A = B08_10m, B = B04_10m
    calc="'((A - B)/(1.0 * A + B) * 10000).astype(int16)'"
    parallel  --colsep ' ' /usr/bin/gdal_calc.py --quiet -A {3} -B {2} --outfile={5} --calc="${calc}" --NoDataValue=-9999 --type='Int16' --creation-option='COMPRESS=LZW' --creation-option='BIGTIFF=YES' --overwrite  ::: "${parameters[@]}" 

elif [ "${index}" == "savi_10m" ]; then

    #NOTE: A = B08_10m, B = B04_10m
    calc="'((A - B)/(A + B + 4280.0) * (10000 + 4280)).astype(int16)'"
    parallel  --colsep ' ' /usr/bin/gdal_calc.py --quiet -A {3} -B {2} --outfile={5} --calc="${calc}" --NoDataValue=-9999 --type='Int16' --creation-option='COMPRESS=LZW' --creation-option='BIGTIFF=YES' --overwrite  ::: "${parameters[@]}" 
