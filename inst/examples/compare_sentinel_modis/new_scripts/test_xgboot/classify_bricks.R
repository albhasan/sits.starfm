#!/usr/bin/Rscript
#-------------------------------------------------------------------------------
# CLASSIFY SENTINEL-2 BRICKS FROM INSIDE DOCKER CONTAINER
#-------------------------------------------------------------------------------
dir.create("~/.sits")
file.copy(from = "/root/data/scripts/config.yml", to = "~/.sits/config.yml")
...
b_type         <- "approx"
brick_dir      <- "/root/data/bricks"
samples_file   <- "/root/data/samples/samples_B_approx_3l.rds"
used_labels    <- sort(unlist(stringr::str_split("Deforestatio,Forest,NonForest", ',')))
#used_bands     <- sort(unlist(stringr::str_split("blue,bnir,green,nnir,red,swir1,swir2", ',')))
used_bands     <- sort(unlist(stringr::str_split("evi,ndmi,ndvi", ',')))
version_number <- 009
out_base_dir   <- "/root/data/results"
...
tmp_directory <- "/root/data/results/tmp"
...
source("/root/data/scripts/util.R")
...
classify <- function(used_bands, used_labels, brick_dir, samples_file,
    ...
    cube <- sits::sits_cube(type = "BRICK",
    ...
    probability_map <- sits::sits_classify(data = cube,
                                           ml_model = model,
                                   # NOTE: xgboost only runs with 1 multicore.
                                           multicores = 1,
                                           memsize = 4,
                                           output_dir = out_dir,
